/**
 * This code was AUTOGENERATED using the kinobi library.
 * Please DO NOT EDIT THIS FILE, instead use visitors
 * to add features, then rerun kinobi to update it.
 *
 * @see https://github.com/metaplex-foundation/kinobi
 */

import {
  Context,
  Pda,
  PublicKey,
  Signer,
  TransactionBuilder,
  transactionBuilder,
} from '@metaplex-foundation/umi';
import {
  Serializer,
  mapSerializer,
  struct,
  u64,
  u8,
} from '@metaplex-foundation/umi/serializers';
import {
  ResolvedAccount,
  ResolvedAccountsWithIndices,
  getAccountMetasAndSigners,
} from '../shared';

// Accounts.
export type PermissionedBurnInstructionAccounts = {
  /** The source account to burn from. */
  account: PublicKey | Pda;
  /** The token mint. */
  mint: PublicKey | Pda;
  /** Authority configured on the mint that must sign any permissioned burn instruction. */
  permissionedBurnAuthority: Signer;
  /** The account's owner/delegate or its multisignature account. */
  authority?: PublicKey | Pda | Signer;
};

// Data.
export type PermissionedBurnInstructionData = {
  discriminator: number;
  permissionedBurnDiscriminator: number;
  /** The amount of tokens to burn. */
  amount: bigint;
};

export type PermissionedBurnInstructionDataArgs = {
  /** The amount of tokens to burn. */
  amount: number | bigint;
};

export function getPermissionedBurnInstructionDataSerializer(): Serializer<
  PermissionedBurnInstructionDataArgs,
  PermissionedBurnInstructionData
> {
  return mapSerializer<
    PermissionedBurnInstructionDataArgs,
    any,
    PermissionedBurnInstructionData
  >(
    struct<PermissionedBurnInstructionData>(
      [
        ['discriminator', u8()],
        ['permissionedBurnDiscriminator', u8()],
        ['amount', u64()],
      ],
      { description: 'PermissionedBurnInstructionData' }
    ),
    (value) => ({
      ...value,
      discriminator: 46,
      permissionedBurnDiscriminator: 1,
    })
  ) as Serializer<
    PermissionedBurnInstructionDataArgs,
    PermissionedBurnInstructionData
  >;
}

// Args.
export type PermissionedBurnInstructionArgs =
  PermissionedBurnInstructionDataArgs;

// Instruction discriminator.
export const permissionedBurnInstructionDiscriminator = 46;

// Instruction.
export function permissionedBurn(
  context: Pick<Context, 'identity' | 'programs'>,
  input: PermissionedBurnInstructionAccounts & PermissionedBurnInstructionArgs
): TransactionBuilder {
  // Program ID.
  const programId = context.programs.getPublicKey(
    'token2022',
    'TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb'
  );

  // Accounts.
  const resolvedAccounts = {
    account: {
      index: 0,
      isWritable: true as boolean,
      value: input.account ?? null,
    },
    mint: { index: 1, isWritable: true as boolean, value: input.mint ?? null },
    permissionedBurnAuthority: {
      index: 2,
      isWritable: false as boolean,
      value: input.permissionedBurnAuthority ?? null,
    },
    authority: {
      index: 3,
      isWritable: false as boolean,
      value: input.authority ?? null,
    },
  } satisfies ResolvedAccountsWithIndices;

  // Arguments.
  const resolvedArgs: PermissionedBurnInstructionArgs = { ...input };

  // Default values.
  if (!resolvedAccounts.authority.value) {
    resolvedAccounts.authority.value = context.identity;
  }

  // Accounts in order.
  const orderedAccounts: ResolvedAccount[] = Object.values(
    resolvedAccounts
  ).sort((a, b) => a.index - b.index);

  // Keys and Signers.
  const [keys, signers] = getAccountMetasAndSigners(
    orderedAccounts,
    'programId',
    programId
  );

  // Data.
  const data = getPermissionedBurnInstructionDataSerializer().serialize(
    resolvedArgs as PermissionedBurnInstructionDataArgs
  );

  // Bytes Created On Chain.
  const bytesCreatedOnChain = 0;

  return transactionBuilder([
    { instruction: { keys, programId, data }, signers, bytesCreatedOnChain },
  ]);
}
