  ...
  return {
    ...
    deserialize: (bytes: Uint8Array, offset = 0) => {
      ...
      if (bytes.length < contentEnd) {
        throw new Error(
          `sizePrefix expected ${Number(
            contentSize
          )} bytes at offset ${contentOffset}.`
        );
      }
